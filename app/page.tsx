...
import { Upload, Shirt, User, Wand2, Settings, Loader2, Image as ImageIcon, CheckCircle2, AlertCircle, ShieldAlert, Layers } from 'lucide-react';
...
export default function Home() {
  ...
  // --- INICIALIZAÇÃO ---
  useEffect(() => {
    // Tenta carregar config do localStorage se existir
  ...
  const uploadToSupabase = async (client, path, file) => {
    const { data, error } = await client.storage.from('studio').upload(path, file);
    if (error) throw error;
    const { data: publicUrl } = client.storage.from('studio').getPublicUrl(path);
    return publicUrl.publicUrl;
  };

  // NOVA FUNÇÃO: Chama nosso Backend Seguro (Passo 2)
  const callBackendAPI = async (garmentUrl, modelUrl) => {
    ...
    return result; // Retorna URL da imagem
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 1. Upload
      const cleanName = (n) => n.replace(/[^a-zA-Z0-9.]/g, '');
      const gPath = `g_${Date.now()}_${cleanName(garmentImage.name)}`;
      const mPath = `m_${Date.now()}_${cleanName(modelImage.name)}`;

      const gUrl = await uploadToSupabase(supabase, gPath, garmentImage);
      const mUrl = await uploadToSupabase(supabase, mPath, modelImage);

      // 2. Processamento (Via Backend)
      ...
      console.error(error);
      setSystemError({ title: "Erro", msg: error.message });
    } finally {
      setLoading(false);
    }
  };

  const fetchGallery = async (client) => {
        ...
        {activeTab === 'studio' && (
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="space-y-6">
              ...
              {[
                { type: 'garment', label: 'Roupa', icon: Shirt, s: garmentPreview, f: setGarmentImage, p: setGarmentPreview },
                { type: 'model', label: 'Modelo', icon: User, s: modelPreview, f: setModelImage, p: setModelPreview }
              ].map(i => (
                <div key={i.type} className="bg-[#18181b] p-6 rounded-xl border border-white/10 relative group">
                  <div className="flex justify-between mb-4 text-sm text-gray-300 font-medium">
                    <span className="flex gap-2 items-center"><i.icon size={16}/> {i.label}</span>
                    {i.s && <CheckCircle2 className="text-green-500" size={16}/>}
                  </div>
                  <div className="h-48 border-2 border-dashed border-white/10 rounded-lg flex items-center justify-center relative bg-black/20 hover:bg-white/5 transition overflow-hidden">
                    {i.s ? <img src={i.s} className="h-full object-contain"/> : <Upload className="text-gray-600"/>}
                    <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={e => {
                      const f = e.target.files[0];
                      if(f) { i.f(f); i.p(URL.createObjectURL(f)); }
                    }}/>
                  </div>
                </div>
              ))}
            ...
            <div className="lg:col-span-2 bg-[#18181b] rounded-xl border border-white/10 p-2 flex flex-col min-h-[500px]">
              <div className="flex-1 bg-black/40 rounded-lg relative flex items-center justify-center overflow-hidden">
                ...
                {resultImage && !loading && <img src={resultImage} className="max-h-full max-w-full object-contain"/>}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {gallery.map(i => (
              <div key={i.id} className="aspect-[3/4] bg-gray-800 rounded overflow-hidden relative group">
                <img src={i.result_url} className="w-full h-full object-cover"/>
